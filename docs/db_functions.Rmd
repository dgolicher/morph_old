...
```{r}
PgLoadRaster<-function(flnm="cold_bay_3857_clip.tiff",x=200,y=200,tabnm="dem",db="brant",srid=3857,path="/home/rstudio/morph/rasters/"){
...
```{r}
PgMakeGrat<-function(dem="dem",minht=-100,maxht=10,xdim=5,ydim=5,db="brant")
{
  require(RODBC)
  con<-odbcConnect(db)
  query<-sprintf("
...
library(dismo)
library(ggplot2)
r<-raster("/home/rstudio/morph/rasters/cold_bay_3857_clip.tiff")
mp <- gmap(r)
plot(mp)
plot(grat,add=T)
head(grat@data)
