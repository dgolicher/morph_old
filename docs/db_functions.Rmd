...
The dimensions of the graticule can be adjusted to be finer than those used when loading the original raster. Note that to ensure an entry in the postgis geometry columns the result needs to be specifically coerced to a polygon type and the srid supplied.

```{r}
PgMakeGrat<-function(dem="dem",srid=3857,minht=-100,maxht=10,xdim=30,ydim=30,db="brant")
{
  require(RODBC)
  con<-odbcConnect(db)
  query<-sprintf("
